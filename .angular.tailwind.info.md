...
2. **Esbuild builder** (`@angular/build:application`) — the modern builder (Angular 19+). It has its own CSS pipeline that **only** reads `.postcssrc.json`. It ignores `postcss.config.js` entirely.
