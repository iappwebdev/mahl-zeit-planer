  ...
  <action>
    **App Shell & Layout:**
    ...
    3. Create `src/app/shared/components/navbar/navbar.component.ts`:
       ...
       **Desktop navigation (hidden on mobile: `hidden md:flex`):**
       ...
       - Center/right: navigation links — "Meine Gerichte" (links to `/gerichte`), "Wochenplan" (links to `/wochenplan`)
       - Far right: "Abmelden" button (visible! not hidden in menu — per locked decision)
       ...
       **Mobile navigation (hidden on desktop: `md:hidden`):**
       ...
       - Three tabs with icons and labels:
         - Gerichte icon (use a plate/utensils Unicode character like 🍽 or simple SVG) + "Gerichte"
         - Wochenplan icon (calendar Unicode like 📅 or simple SVG) + "Wochenplan"
         - Abmelden icon (logout/door Unicode like 🚪 or simple SVG) + "Abmelden"
    ...
    **Complete Route Configuration:**
    ...
    6. Update `src/app/app.routes.ts` with the full route tree:
       ...
       - `email-bestaetigen` has no guard — accessible always (user just registered, not yet authenticated)

    **Use angular 21 control flow syntax** (`@if`, `@for`) not `*ngIf`/`*ngFor`.
  </action>
  <verify>
    Run `ng serve` — app starts without errors.
    Run `ng build` — production build succeeds.
    Navigate to localhost:4200 — redirects to /anmelden (not logged in).
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <action>
    ...
    5. Authenticated app:
       ...
       b. Navigate to "Wochenplan" — see "Kommt bald" placeholder
       c. Navigate back to "Meine Gerichte" — see "Kommt bald" placeholder
    6. Logout:
    ...
    8. Password reset:
       ...
       b. Enter email and submit — see confirmation message
    9. Visual check:
  ...
  <done>Complete Phase 1 auth flow and responsive app shell verified by human — register, login, logout, password reset, navigation, placeholders, and German UI all working correctly</done>
</task>
