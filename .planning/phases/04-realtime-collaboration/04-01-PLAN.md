...
- `household_id UUID NOT NULL REFERENCES public.households(id) ON DELETE CASCADE`
- `user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE`
- `display_name TEXT` — populated by trigger via SELECT from profiles; used in Realtime payload for person-named toasts without extra JOIN
...
- Create activity_log triggers on dishes and meal_assignments for automatic logging (INSERT/UPDATE/DELETE on dishes writes to activity_log; INSERT/UPDATE/DELETE on meal_assignments writes to activity_log). These triggers must be `SECURITY DEFINER` functions that:
  1. SELECT `display_name` FROM `public.profiles` WHERE `id = NEW.user_id` (or OLD.user_id on DELETE)
  2. Write the resolved `display_name` into the `activity_log` INSERT alongside `household_id`, `user_id`, `action`, `entity_type`, `entity_name`
  3. Only fire when `household_id IS NOT NULL` on the affected row

  This is critical: Supabase Realtime `postgres_changes` sends only actual row columns — no JOINs are possible in the payload. By writing `display_name` directly into `activity_log`, the Realtime payload carries it, enabling "Lisa hat Lachs hinzugefuegt" toasts without any additional lookup in the client.
...
**CRITICAL:** Use `(SELECT auth.uid())` wrapper in ALL RLS policies (established project pattern, 94-99% performance improvement).
  ...
  <verify>
    ...
    - activity_log table schema contains `display_name TEXT` column
    - activity_log trigger function contains `SELECT display_name FROM` (or equivalent lookup from profiles) and writes it into the INSERT statement
  </verify>
  <done>
