...
must_haves:
  truths:
    ...
    - "A subtle toast notification shows what changed and who changed it (e.g. 'Lisa hat Lachs hinzugefuegt')"
...
In constructor, add an `effect()` watching `this.realtime.dishChange()`:
```typescript
effect(() => {
  ...
  if (change) {
    ...
    // Show toast if the change was made by someone else
    const currentUserId = /* get from SupabaseService or signal */;
    if (change.new?.user_id !== currentUserId && change.old?.user_id !== currentUserId) {
      const name = change.new?.name || change.old?.name || 'Gericht';
      let message = '';
      switch (change.eventType) {
        case 'INSERT': message = `${name} wurde hinzugefuegt`; break;
        case 'UPDATE': message = `${name} wurde geaendert`; break;
        case 'DELETE': message = `${name} wurde entfernt`; break;
      }
      if (message) {
...
Note: The toast should ideally include the person's name (per CONTEXT.md: "Lisa hat Lachs hinzugefuegt"). The `postgres_changes` payload includes the row data but not the user's display name. Options:
- Use the `activity_log` table (populated by triggers with display name) as the toast source instead.
- Or keep simple: just show dish name without person name in the Realtime toast. The activity feed (Task 2) will show the full "Lisa hat..." entries.
**Decision:** Use simple dish-name-only toasts from Realtime for instant feedback. Activity feed provides the full "who did what" context. This avoids an extra lookup per Realtime event.

**C. Update MealPlanComponent** (`src/app/features/meal-plan/meal-plan.component.ts`):

Add `private realtime = inject(RealtimeService)`.

In constructor, add an `effect()` watching `this.realtime.assignmentChange()`:
```typescript
effect(() => {
  ...
  if (change) {
    ...
    // meal_assignments don't have user_id directly, so we can't easily filter own changes.
    // Show a generic toast only when the changed plan matches the currently viewed week.
    // The loadWeek() call refreshes the UI; toast is supplementary.
    this.snackBar.open('Wochenplan wurde aktualisiert', '', { duration: 3000 });
  }
});
```

**Important:** The `effect()` for Realtime must NOT conflict with the existing `effect()` that watches `currentWeekStart` for navigation. They are independent — one reacts to week changes, the other to external data changes. Both call `loadWeek()` which is safe to call multiple times (it replaces the signal value).

**D. Update LayoutComponent** (`src/app/shared/components/layout/layout.component.ts`):

Add `inject(RealtimeService)` — this ensures the RealtimeService is instantiated when the authenticated layout loads (since it's providedIn: 'root', it's lazy; injecting it in LayoutComponent guarantees it starts). The service's internal `effect()` on `householdId` handles subscribe/unsubscribe automatically.

If LayoutComponent already has init logic (from Plan 02's invite token check), add the RealtimeService inject alongside it.
  </action>
  <verify>
    - `src/app/core/services/realtime.service.ts` exists and exports RealtimeService
    - RealtimeService has `dishChange`, `assignmentChange`, `memberChange` signals
    ...
    - DishesComponent shows snackbar toast on external dish changes
    - MealPlanComponent injects RealtimeService and has `effect()` watching `assignmentChange`
    - MealPlanComponent shows snackbar toast on assignment changes
